...
// A stable identity for a question, independent of where it sits in the file.
// The array index cannot serve this purpose: appending is safe, but inserting
// or removing a question would shift every later index and corrupt the
// persisted "already seen" set. cyrb53 gives a 53-bit hash, so collisions
// across a few thousand questions are vanishingly unlikely.
function cyrb53(str) {
  let h1 = 0xdeadbeef
  let h2 = 0x41c6ce57
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then((data) => {
        ...
        const questions = valid.map((q, i) => {
          ...
          return {
            id: i,
            uid: cyrb53(`${q.source || ''}|${q.question}`),
            question: q.question,
